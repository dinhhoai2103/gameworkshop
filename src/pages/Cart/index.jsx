...
function Cart({
  ...
  const totalPrice = () => {
    ...
    cartData.map((item) => {
      if (item.isPay === false) {
        return (
          JSON.parse(localStorage.getItem(localStorage.key(0))).id ===
            item.idUser && (itemPrice = item.price * item.soluong + itemPrice)
        );
      }
    });
    return itemPrice;
    ...
    <div className="container">
      <div className="cart-main">
        <div className="container-title">
          <span className="sp">
            <h2 className="mr-2">Giỏ Hàng </h2>
            {cartData && <span>{cartData.length} Sản phẩm</span>}
          </span>
